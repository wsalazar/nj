...
jQuery(document).ready(function($){
	...
	$('#footer #footer-left').append('<?php echo $footer?>');
...
<?php	
		$this->handle_wp_logo_in_admin_bar($header_logo,$header_logo_width);
	}	
	
	function wp_head(){
		global $wlb_plugin;
		if(  '1'!=$wlb_plugin->get_option('use_branding_customization') )return true;
		$vars = array('header_logo','header_logo_width');
		foreach($vars as $var){
			$$var = $wlb_plugin->get_option($var);
			if(in_array($var,array('header_logo'))){
				$$var = str_replace("{pluginurl}", WLB_URL, $$var);
	...
	function handle_wp_logo_in_admin_bar($header_logo,$header_logo_width){
?>
