<h2 class="nav-tab-wrapper big-tabs-tabs">
	...
	<a class="nav-tab" href="#tab-feed">Feed</a>
...
<form method="post">
	...
	<div class="big-tabs-container">
		...
		<div class="big-tab" id="tab-general-content">
			...
			$form = array(	
				array(
					...
					'description' => 'If you are a member of the Headway Affiliate program (if not, you should definitely <a href="http://www.headwaythemes.com/affiliate-program/" target="_blank">sign up now!</a>), you can paste your affiliate link (found at the top of the affiliate panel) and earn money when someone purchases Headway through your affiliate link.  <strong>Do NOT put HTML in this field.</strong>'
				)
			);
					...
					<div id="seo-templates-hidden-inputs">
						...
						foreach (HeadwaySEO::output_layouts_and_defaults() as $page => $defaults) {
							...
							foreach ($seo_template_inputs as $input) {
								...
								$page_options = Headway::get($page, $seo_options, array());
								$value = Headway::get($input, $page_options, $default);

								echo '<input type="hidden" id="seo-' . $page . '-' . $input . '"' . $name_attr . ' value="' . $value . '" />';
		...
		<div class="big-tab" id="tab-feed-content">
			...
			$form = array(	
				...
				array(
					'id' => 'feed-exclude-cats',
					'type' => 'select',
					'multiple' => true,
					'label' => 'Exclude Categories From Feed',
					'options' => $category_options,
					'value' => HeadwayOption::get('feed-exclude-cats', false, array()),
					'description' => 'With Headway you can exclude certain categories and specific posts from the feeds. You can choose which categories you would like to exclude using the select box below, and you can exclude specific posts on the post edit panel. To select multiple categories, hold Command (or Control if you are using a PC).'
				)
			);
		...
		<div class="big-tab" id="tab-visual-editor-content">
			...
			$form = array(	
				array(
					...
					'description' => 'If you ever feel that the tooltips are too invasive in the visual editor, you can disable them here.  Tooltips are the black speech bubbles that appear to assist you when you are not sure what an option is.'
				),
