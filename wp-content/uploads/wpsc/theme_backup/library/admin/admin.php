...
class HeadwayAdmin {
	...
	public static function form_action_save() {
		...
		if ( !Headway::post('headway-submit', false))
			return false;
			
		if ( !wp_verify_nonce(Headway::post('headway-admin-nonce', false), 'headway-admin-nonce') ) {
		...
		foreach ( Headway::post('headway-admin-input', array()) as $option => $value ) {
	...
	public static function form_action_reset() {
		...
		//Form action for all Headway configuration panels.  Not in function/hook so it can load before everything else.
		if ( !Headway::post('reset-headway', false) )
			return false;
			
		//Verify the nonce so other sites can't maliciously reset a Headway installation.
		if ( !wp_verify_nonce(Headway::post('headway-reset-nonce', false), 'headway-reset-nonce') ) {
	...
	public static function form_action_maintenance() {
		...
		//Form action for all Headway configuration panels.  Not in function/hook so it can load before everything else.
		if ( !Headway::post('headway-maintenance-nonce', false) )
			return false;
			
		//Check the nonce for security
		if ( !wp_verify_nonce(Headway::post('headway-maintenance-nonce', false), 'headway-maintenance-nonce') ) {
		...
		//Handle block repair
		if ( Headway::post('repair-blocks', false) ) {
	...
	public static function activation() {
		...
		if ( !is_admin() || !Headway::get('activated') )
			return false;
		...
		switch ( $menu_setup ) {
			...
			case 'tools':
				$parent_menu = array(
					'id' => 'tools',
					'name' => 'Tools',
					'callback' => array('HeadwayAdminPages', 'tools')
				);
			break;
	...
	public static function enqueue() {
		...
		if ( isset($_GET['page']) && strpos($_GET['page'], 'headway') !== false ) {
		...
		if ( $pagenow === 'update-core.php' ) {
			...
			wp_localize_script('headway_admin_js', 'HeadwayUpdateInfo', array( 
				'updateAvailable' => HeadwayUpdater::is_update_available(),
				'info' => json_encode(HeadwayUpdater::get_update_info())
			));
