<?php
headway_register_admin_meta_box('HeadwayMetaBoxAlternateTitle');
class HeadwayMetaBoxAlternateTitle extends HeadwayAdminMetaBoxAPI {
	
	protected $id = 'alternate-title';
	
	protected $name = 'Alternate Title';
...
class HeadwayMetaBoxPostThumbnail extends HeadwayAdminMetaBoxAPI {
	...
	protected $inputs = array(
		'position' => array(
			...
			'description' => 'Set the position of the featured image for this post.',
			'default' => 'left',
...
class HeadwayMetaBoxSEO extends HeadwayAdminMetaBoxAPI {
	...
	protected $inputs = array(
		...
		'noindex' => array(
			'id' => 'noindex',
			'group' => 'seo',
			'name' => '<code>noindex</code> this enry.',
			'type' => 'checkbox',
	...
	protected function modify_arguments($post = false) {
		...
		//Do not use this box if any SEO plugin is active
		if ( HeadwaySEO::plugin_active() ) {
			
			switch ( HeadwaySEO::plugin_active() ) {
				
				case 'aioseop':
					$this->info = 'Headway has detected that you are using the All In One SEO pack plugin.  In order to reduce conflicts, Headway\'s SEO functionality has been disabled.';
				break;
				
				case 'wpseo':
					$this->info = 'Headway has detected that you are using Yoast\'s WordPress SEO plugin.  In order to reduce conflicts, Headway\'s SEO functionality has been disabled.';
				break;
		...
		} elseif ( get_option('page_on_front') == Headway::get('post') && get_option('show_on_front') == 'page' ) {
		...
		$seo_templates = Headway::get('single-' . $current_screen->id, $seo_templates_query, array());
		
		$title_template = str_replace(array('%sitename%', '%SITENAME%'), get_bloginfo('name'), Headway::get('title', $seo_templates));
				
		echo '<input type="hidden" id="title-seo-template" value="' . $title_template . '" />';
				
		$this->inputs['noindex']['default'] = Headway::get('noindex', $seo_templates);
		$this->inputs['nofollow']['default'] = Headway::get('nofollow', $seo_templates);
		$this->inputs['noarchive']['default'] = Headway::get('noarchive', $seo_templates);
		$this->inputs['nosnippet']['default'] = Headway::get('nosnippet', $seo_templates);
		$this->inputs['noodp']['default'] = Headway::get('noodp', $seo_templates);
		$this->inputs['noydir']['default'] = Headway::get('noydir', $seo_templates);
