...
if(! defined('MEMORY_LIMIT') )				define ('MEMORY_LIMIT', WP_MAX_MEMORY_LIMIT);			// Set PHP memory limit
...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', HEADWAY_CACHE);			// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
class timthumb {
	...
	protected function calcDocRoot($useDocumentRoot = true){
		if ( $useDocumentRoot ) {
	...
	protected function getLocalImagePath($src){
		...
		//Try docRoot calculated a different way
		$altDocRoot = $this->calcDocRoot(false);
		if(file_exists ($altDocRoot . '/' . $src)) {
			$this->debug(3, "Found file as " . $altDocRoot . '/' . $src);
			$real = $this->realpath($altDocRoot . '/' . $src);
			if(stripos($real, $altDocRoot) === 0){
				return $real;
			} else {
				$this->debug(1, "Security block: The file specified occurs outside the document root.");
				//allow search to continue
