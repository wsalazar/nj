...
class HeadwayHead {
	...
	public static function register_files() {
		...
		if ( current_theme_supports('headway-reset-css') || current_theme_supports('headway-grid') )
			$general_css_fragments[] = HEADWAY_LIBRARY . '/media/css/reset.css';
		
		if ( current_theme_supports('headway-grid') )
			$general_css_fragments[] = HEADWAY_LIBRARY . '/media/css/grid.css';
					
		if ( current_theme_supports('headway-block-basics-css') )
			$general_css_fragments[] = HEADWAY_LIBRARY . '/media/css/block-basics.css';
			
		if ( current_theme_supports('headway-content-styling-css') ) {
			
			$general_css_fragments[] = HEADWAY_LIBRARY . '/media/css/content-styling.css';
			$general_css_fragments[] = HEADWAY_LIBRARY . '/media/css/alerts.css';
		...
		if ( current_theme_supports('headway-grid') || current_theme_supports('headway-design-editor') || current_theme_supports('headway-live-css') )
			$general_css_dependencies[] = HEADWAY_LIBRARY . '/media/dynamic/style.php';
			
		//Allow filters to be applied to the general CSS fragments/dependencies before the count is made
		$general_css_fragments = apply_filters('headaway_general_css', $general_css_fragments);
		$general_css_dependencies = array_unique(apply_filters('headaway_general_css_dependencies', $general_css_dependencies));
		...
		if ( HeadwayRoute::is_visual_editor_iframe() ) {
			...
			HeadwayCompiler::register_file(array(
				...
				'format' => 'css',
				'fragments' => array(
					HEADWAY_LIBRARY . '/visual-editor/css/editor-iframe.css',
		...
		if ( HeadwayResponsiveGrid::is_active() ) {
			...
			HeadwayCompiler::register_file(array(
				...
				'dependencies' => array(
					HEADWAY_LIBRARY . '/media/dynamic/responsive-grid.php'
				)
			));
...
<![endif]-->');
	...
	public static function favicon() {
		...
		if ( HeadwayOption::get('favicon') )
			echo "\n\n<!-- Favicon -->\n" . '<link rel="shortcut icon" type="image/ico" href="' . HeadwayOption::get('favicon') . '" />' . "\n\n\n";
