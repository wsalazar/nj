...
/* IFRAME FUNCTIONS */
	...
	loadIframe = function(callback) {
		...
		iframeURL = Headway.homeURL + '?ve-iframe=true&ve-iframe-layout=' + Headway.currentLayoutInUse + '&ve-iframe-mode=' + Headway.mode + '&rand=' + Math.floor(Math.random() * 100000001);
	...
	defaultIframeLoad = function() {
		...
		if ( Headway.currentLayout.indexOf('template-') === -1 && Headway.currentLayoutInUse.indexOf('template-') === 0 ) {
...
/* LAYOUT FUNCTIONS */
	switchToLayout = function(layoutNode, reloadIframe, showSwitchNotification) {
		...
		Headway.currentLayoutInUse = layout_id;
		Headway.currentLayoutName = layout_name;
				
		//Check if the layout node has a template assigned to it.  
		//If it does, change the currentLayoutInUse so the message is displayed upon iframe reload
		...
		if ( typeof possibleTemplateID != 'undefined' && possibleTemplateID != 'none' )
			Headway.currentLayoutInUse = possibleTemplateID;
	...
	openImageUploader = function(callback) {
		...
		if ( !boxExists('input-image') ) {
			...
			var settings = {
				...
				src: Headway.adminURL + '/media-upload.php?type=image&amp;TB_iframe=true&amp;post_id=' + Headway.currentLayout,
				load: iframeLoad,
				width: 670,
				height: 500,
				center: true,
