...
class HeadwayVisualEditorDisplay {
	...
	public static function init() {
		...
		add_action('wp_print_scripts', array(__CLASS__, 'search_nodes_script'));
	...
	public static function enqueue_scripts() {
		...
		HeadwayCompiler::register_file(array(
			...
			'format' => 'js',
			'fragments' => array(
				HEADWAY_LIBRARY . '/media/js/itstylesheet.js',

				HEADWAY_LIBRARY . '/media/js/jquery.ui.js',
				HEADWAY_LIBRARY . '/media/js/jquery.iframe.js',
				HEADWAY_LIBRARY . '/media/js/jquery.qtip.js',
				HEADWAY_LIBRARY . '/media/js/jquery.cookie.js',
				HEADWAY_LIBRARY . '/media/js/jquery.masonry.js',
				HEADWAY_LIBRARY . '/media/js/jquery.animate-shadow.js',
				HEADWAY_LIBRARY . '/media/js/jquery.scrollbarpaper.js',
				HEADWAY_LIBRARY . '/media/js/jquery.tabby.js',
				
				HEADWAY_LIBRARY . '/media/js/colorpicker/colorpicker.js',
				
				HEADWAY_LIBRARY . '/media/js/codemirror/codemirror.js',
				HEADWAY_LIBRARY . '/media/js/codemirror/languages/css.js',
				
				HEADWAY_LIBRARY . '/visual-editor/js/editor.js',
				HEADWAY_LIBRARY . '/visual-editor/js/editor.inputs.js',
				HEADWAY_LIBRARY . '/visual-editor/js/editor.functions.js',

				HEADWAY_LIBRARY . '/visual-editor/js/editor.widget.grid.js',

				HEADWAY_LIBRARY . '/visual-editor/js/editor.mode.grid.js',
				HEADWAY_LIBRARY . '/visual-editor/js/editor.mode.design.js',
				HEADWAY_LIBRARY . '/visual-editor/js/editor.mode.manage.js'
			)
		));
	...
	public static function enqueue_styles() {
		...
		HeadwayCompiler::register_file(array(
			...
			'format' => 'css',
			'fragments' => array(
				HEADWAY_LIBRARY . '/visual-editor/css/editor-tooltips.css',
				HEADWAY_LIBRARY . '/visual-editor/css/editor.css',
				HEADWAY_LIBRARY . '/visual-editor/css/editor-inputs.css',
				HEADWAY_LIBRARY . '/visual-editor/css/editor-design.css',
				
				HEADWAY_LIBRARY . '/media/js/codemirror/codemirror.css',
				HEADWAY_LIBRARY . '/media/js/codemirror/theme-default.css'
			),
			'enqueue' => false
		));
	...
	public static function add_visual_editor_js_vars() {
		...
		wp_localize_script('headway-visual-editor-js', 'Headway', array( 
			...
			'currentLayoutInUse' => HeadwayLayout::get_current_in_use(),
			'currentLayoutName' => HeadwayLayout::get_name(HeadwayLayout::get_current()),
			'currentLayoutInUseName' => HeadwayLayout::get_name(HeadwayLayout::get_current_in_use()),
			'availableBlockID' => HeadwayBlocksData::get_available_block_id(),
			'headwayURL' => get_template_directory_uri(),
			'childThemeURL' => HEADWAY_CHILD_THEME_URL,
			'siteURL' => site_url(),
			'homeURL' => home_url(),
			'adminURL' => admin_url(),
			...
			'ranTour' => HeadwayOption::get('ran-tour', false, false),
	...
	public static function search_nodes_script() {
		
		echo "\n" . '<script type="text/javascript">' . "\n";
		
			echo 'HEADWAY_VE_SEARCH_OBJECTS = [' . "\n";
			
				$search_objects = '';
									
				foreach ( $GLOBALS['headway_search_panels'] as $panel => $options ) {
										
					$search_objects .= '{type:\'tab\', label: \'' . $options['name'] . '\', value: \'' . $panel . '\', mode: \'' . $options['mode'] . '\'},' . "\n";
					
					//If the main tab group doesn't have any sub-tabs, go to the next
					if ( !isset($options['tabs']) )
						continue;
					
					foreach ( $options['tabs'] as $tab => $options ) {
						
						$search_objects .= '{type:\'sub-tab\', label: \'' . $options['name'] . '\', value: \'' . $tab . '\', tab: \'' . $panel . '\'},' . "\n";

						//If the sub-tab doesn't have any inputs, go to the next
						if ( !isset($options['inputs']) )
							continue;
						
						foreach ( $options['inputs'] as $input => $options ) {
							
							$search_objects .= '{type:\'input\', label: \'' . $options['label'] . '\', value: \'' . $input . '\', tab: \'' . $panel . '\', subTab: \'' . $tab . '\',},' . "\n";
				...
				//Remove last comma and return objects
				echo substr($search_objects, 0, -1);
	...
	public static function panel_top_right() {
		...
		<li id="search">
			<span class="tooltip-bottom-right" title="Looking for a specific option?  Try searching here!">Search</span>
	...
	public static function panel_top_mode_buttons() {
		...
		switch ( HeadwayVisualEditor::get_current_mode() ) {
			...
			case 'grid':
				...
				echo '<span class="mode-button tooltip" id="show-preview" title="Click to hide the grid and show how the website looks.  &lt;strong&gt;&lt;em&gt;Note:&lt;/strong&gt; all changes must be saved before they will show in the preview.&lt;/em&gt;">Show Preview</span>';			
				//echo '<span class="mode-button" id="insert-wrapper-break">Insert Wrapper Break</span>';
			...
			case 'design':
				...
				//echo '<span class="mode-button" id="toggle-inspector">Enable Inspector</span>';
	...
	public static function block_type_popup() {
		...
		echo "\n". '<div id="block-type-popup" style="display:none;">' . "\n";
			...
			echo '<ul>'. "\n";			
				...
				//Core blocks
				foreach($block_types['core'] as $block => $block_options) {
					
					echo '<li id="block-type-' . $block . '" style="background-image: url(' . $block_options['url'] . '/icon.png);">' . $block_options['name'] . '</li>' . "\n";
					
				}
				
				if ( count($block_types['plugins']) !== 0 ) {
					
					echo '<li id="more-blocks">' ." \n";
					...
					//Plugin blocks
					foreach($block_types['plugins'] as $block => $block_options) {
		...
		echo '<ul>' . "\n";
			...
			foreach ( $pages as $id => $children ) {
				...
				if ( HeadwayVisualEditor::get_current_mode() !== 'grid' ) {
					...
					if ( Headway::get('customized', $status, false) === false || Headway::get('template', $status, false) !== false ) {
				...
				echo "\n" . '<li class="' . implode(' ', array_filter($class)) . '">' . "\n";
					...
					echo "\n". '
						...
						<span layout_id="' . $id . '" class="layout layout-page">
							<strong>' . HeadwayLayout::get_name($id) . '</strong>

							<span class="status status-template" template_id="' . $template_id . '">' . $template_name . '</span>							
	...
	public static function list_templates() {
		...
		echo '<ul>' . "\n";
				...
				echo "\n" . '<li class="' . implode(' ', array_filter($class)) . '">' . "\n";
					...
					echo "\n". '
						<span layout_id="template-' . $id . '" class="layout layout-template">
							<strong class="template-name">' . $name . '</strong>
			...
			//Add template input
			echo '
				<li class="layout-item add-template-container">
					<span class="layout">
	...
	public static function is_any_layout_child_customized($children) {
		...
		foreach ( $children as $id => $grand_children ) {
			...
			if ( Headway::get('customized', $status) && !Headway::get('template', $status) )
				return true;
