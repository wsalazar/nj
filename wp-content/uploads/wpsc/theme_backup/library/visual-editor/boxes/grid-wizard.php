...
?>
		<ul id="grid-wizard-tabs" class="tabs">
			...
			if ( $pages_to_clone_select_options !== '' )
				echo '<li><a href="#grid-wizard-tab-clone-page">Clone Existing Page</a></li>';
...
<?php
	...
	static function get_blocks_select_options_for_mirroring($block_type) {
		...
		foreach ( $blocks as $block_id => $layout_id ) {
			...
			$return .= '<option value="' . $block['id'] . '">' . Headway::get('alias', $block['settings'], $default_name) . ' &ndash; ' . $layout_name . '</option>';  
