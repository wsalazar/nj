...
class HeadwayElementInputs {
	...
	function box($args) {
		...
		/* Create the box */
		echo '<div class="design-editor-box">';
			...
			echo '<ul class="design-editor-box-content">';
				...
				foreach($properties as $property_id => $options) {
					...
					if ( Headway::get($property_id, $property_values) ) {
					...
					echo '<li class="' . $uncustomized_property_class . '">';
	...
	function input_integer($options, $id) {
		...
		echo '<input type="text" value="' . $options['value'] . '" /><span class="unit">px</span>';	
			...
			if ( is_array($fonts) ) {
				...
				foreach ( $fonts as $group => $content ) {
					...
					foreach ( $content as $value => $text ) {
						...
						$font_stack = isset($headway_fonts[$value]) ? $headway_fonts[$value] : $value;
