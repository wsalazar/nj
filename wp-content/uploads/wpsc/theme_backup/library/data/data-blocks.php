...
class HeadwayBlocksData {
	...
	public static function get_block($block, $use_mirrored = false) {
		...
		if ( is_array($block) ) {
			...
			if ( (!isset($block['id']) || !self::block_exists($block['id'])) && !Headway::get('new', $block, false) )
				return null;
		...
		/* Fetch the block based off of ID */
			...
			$layout_blocks = self::get_blocks_by_layout($blocks_by_id[$block]['layout']);
	...
	public static function get_block_name($block) {
		...
		return Headway::get('alias', $block['settings'], $default_name);
	...
	public static function get_block_width($block) {
		...
		$block_grid_width = Headway::get('width', $block['dimensions'], null);
	...
	public static function get_block_height($block) {
		...
		$block_grid_height = Headway::get('height', $block['dimensions'], null);
	...
	public static function get_block_setting($block, $setting, $default = null) {
		...
		return Headway::fix_data_type($block['settings'][$setting]);
	...
	public static function is_block_mirrored($block, $return_block_id = false) {
		...
		if ( $block && $mirrored_block_id = Headway::get('mirror-block', $block['settings']) ) {

			$mirrored_block = self::get_block($mirrored_block_id);

			if ( !$mirrored_block || Headway::get('mirror-block', $mirrored_block['settings']) )
				return false;
