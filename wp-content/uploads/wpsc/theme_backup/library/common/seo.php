...
class HeadwaySEO {
	...
	public static function output_title($title) {
		...
		$seo_templates = Headway::get(HeadwaySEO::current_seo_layout(), $seo_templates_query, array());
		
		if ( HeadwayLayoutOption::get(false, 'title', 'seo', false) )
			$title = HeadwayLayoutOption::get(false, 'title', 'seo', false);
			
		elseif ( Headway::get('title', $seo_templates) )
			$title = Headway::get('title', $seo_templates);
	...
	public static function output_meta() {
		...
		elseif ( Headway::get('description', $seo_templates) )
			$meta .= "\n" . '<meta name="description" content="' . self::parse_seo_variables(Headway::get('description', $seo_templates)) . '" />';
	...
	public static function is_seo_checkbox_enabled($option, $layout = false) {
		...
		if ( HeadwayLayoutOption::get($layout, $option, 'seo', null) === null && !Headway::get($option, $seo_templates) )
			return false;
	...
	public static function get_seo_templates($layout = false) {
		...
		if ( $layout )
			return Headway::get(self::current_seo_layout(), $seo_templates_query, array());
		else
