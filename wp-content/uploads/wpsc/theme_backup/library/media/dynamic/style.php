...
class HeadwayDynamicStyle {
	...
	static function grid() {
		...
		static function responsive_grid() {
			...
			$return .= '.column { margin-left: ' . round($resp_single_column_margin, 3) . '%; }';
			...
			for ( $i = 1; $i <= $grid_number; $i++ ) {
				...
				/* Output */
				$return .= '.grid-width-' . $i . ' { width: ' . round($resp_grid_width - $resp_single_column_margin, 3) . '%; }';					
				$return .= '.grid-width-' . $i . '.column-1 { width: ' . round($resp_grid_width, 3) . '%; }';					
				
				if ( $i < 24 ) {
					
					$return .= '.grid-left-' . $i . ' { margin-left: ' . round($resp_grid_left_margin, 3) . '%; }';
					$return .= '.grid-left-' . $i . '.column-1 { margin-left: ' . round($resp_grid_left_margin - $resp_single_column_margin, 3) . '%; }';
					
				}
				
				/* Responsive Sub Columns */
				$return .= '.grid-width-' . $i . ' .sub-column { margin-left: ' . round($sub_column_single_margin, 3) . '%; }';
				...
				for ( $sub_column_i = 1; $sub_column_i < $i; $sub_column_i++ ) {
					...
					$return .= '.grid-width-' . $i . ' .sub-column.grid-width-' . $sub_column_i . ' { width: ' . round($sub_column_width - $sub_column_single_margin, 3) . '%; }';
					$return .= '.grid-width-' . $i . ' .sub-column.grid-width-' . $sub_column_i . '.column-1 { width: ' . round($sub_column_width, 3) . '%; }';
					
					$return .= '.grid-width-' . $i . ' .sub-column.grid-left-' . $sub_column_i . ' { margin-left: ' . round($sub_column_margin, 3) . '%; }';
					$return .= '.grid-width-' . $i . ' .sub-column.grid-left-' . $sub_column_i . '.column-1 { margin-left: ' . round($sub_column_margin - $sub_column_single_margin, 3) . '%; }';
		...
		static function block_heights() {
			...
			foreach ( $blocks as $block ) {
				...
				if ( Headway::get('fixed-height', Headway::get($block['type'], $block_types), false) !== true )
	...
	static function wrapper() {
		...
		$horizontal_padding = HeadwayOption::get('wrapper-horizontal-padding', 'general', 15) . 'px';
		$vertical_padding = HeadwayOption::get('wrapper-vertical-padding', 'general', 15) . 'px';
	...
	static function live_css() {
		...
		if ( Headway::get('visual-editor-open') )
			return null;
