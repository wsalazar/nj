...
class HeadwayBreadcrumbsBlock extends HeadwayBlockAPI {
	...
	function setup_elements() {
		...
		$this->register_block_element(array(
			...
			'selector' => 'p a',
			'properties' => array('fonts'),
			'inherit-location' => 'inherit-hyperlink'
		));
	...
	function content($block) {
		...
		echo '<p class="breadcrumbs">';
			...
			elseif ( is_author() ) {
				...
				$authordata = get_query_var('author_name') ? get_userdatabylogin(get_query_var('author_name')) : get_userdata(get_query_var('author'));
					
				echo $blog . ' &raquo; <span id="current-breadcrumb">' . __('Author Archives:', 'headway') . ' ' . $authordata->display_name . '</span>';
