...
class HeadwayContentBlockDisplay {
	...
	function remove_hooks() {
		...
		//Since this function is called in the constructor, the $content_block_display variable is still set to the previous content block.
		
		global $content_block_display;		
		
		if ( !class_exists('pluginbuddy_loopbuddy') ) {
			
			remove_filter('the_content_more_link', array($content_block_display, 'more_link'));			
		
			remove_filter('excerpt_more', '__return_false');
			remove_filter('get_the_excerpt', array($content_block_display, 'add_excerpt_more_link'));
			
		}
					
		remove_filter('the_content', array($content_block_display, 'nofollow_links_in_post'));
	...
	function display($args = array()) {
		...
		if ( Headway::get('ve-live-content-query', $this->block, false) || HeadwayRoute::is_visual_editor_iframe() ) 
		...
		if ( is_404() && $this->get_setting('mode', 'default') == 'default' && !Headway::get('ve-live-content-query', $this->block, false) )
	...
	function show_query_title($echo = true) {
		...
		if ( is_author() ) {
			...
			$author_url = esc_url(get_the_author_meta('google_profile'));
			
			$return .= '<h1 class="archive-title author-title">';
			
				if ( $author_url )
					$return .= sprintf(__( 'Author Archives: %s', 'headway'), '<span class="vcard"><a class="url fn n" href="' . $author_url . '" title="' . esc_attr(get_the_author()) . '" rel="author">' . get_the_author() . '</a></span>');
					
				else
					$return .= sprintf(__( 'Author Archives: %s', 'headway'), '<span class="vcard">' . get_the_author() . '</span>');
		...
		if ( is_post_type_archive() ) {
			...
			$return .= '<h1 class="archive-title post-type-archive-title">';
				$return .= apply_filters('headway_post_type_archive_title', $queried_object->name);
			$return .= '</h1><!-- .archive-title -->';
	...
	function setup_query() {
		...
		//custom query.
		if ( $this->get_setting('mode', 'default') == 'default' && !Headway::get('ve-live-content-query', $this->block, false) ) {
		...
		//The mode is custom query so we have to set it all up.
			...
			if ( $this->get_setting('mode', 'default') == 'custom-query' ) {
				...
				if ( $this->get_setting('fetch-page-content', false) ) {
					...
					//Post Limit
					if ( $this->get_setting('number-of-posts', 10) && $this->get_setting('paginate', true) ) 
						$query_options['posts_per_page'] = $this->get_setting('number-of-posts', 10);

					elseif ( $this->get_setting('number-of-posts', 10) && !$this->get_setting('paginate', true) ) 
						$query_options['showposts'] = $this->get_setting('number-of-posts', 10);
					//End Post Limit
	...
	function display_entry($args = array()) {
		...
		$post_date = sprintf( '<a href="%1$s" title="%2$s" rel="bookmark"><span class="entry-date">%3$s</span></a>',
			get_permalink(),
			esc_attr( get_the_time() ),
			get_the_date()
		);
		
		$post_vcard = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
			get_author_posts_url( get_the_author_meta( 'ID' ) ),
			sprintf( esc_attr__( 'View all posts by %s', 'headway' ), get_the_author() ),
			get_the_author()
		);
		...
		if ( $this->get_setting('show-entry', true) ) {
			...
			echo '<div id="post-' . $post_id . '" class="' . $post_class . '">';
					...
					if ( has_post_thumbnail() && $this->get_setting('show-post-thumbnails', true) ) {
						...
						$position = HeadwayLayoutOption::get($post_id, 'position', 'post-thumbnail', 'left');

						$thumbnail_size = $this->get_setting('post-thumbnail-size', 125);

						$thumbnail_id = get_post_thumbnail_id();  
						$thumbnail_object = wp_get_attachment_image_src($thumbnail_id, array($thumbnail_size, $thumbnail_size));  

						if ( $this->get_setting('crop-post-thumbnails', true) ) {

							$thumbnail_url = headway_thumbnail($thumbnail_object[0], $thumbnail_size, $thumbnail_size);
							$thumbnail_width = $thumbnail_size;
							$thumbnail_height = $thumbnail_size;

						} else {

							$thumbnail_url = $thumbnail_object[0];
							$thumbnail_width = $thumbnail_object[1];
							$thumbnail_height = $thumbnail_object[2];

						}

						echo '
							<a href="' . $post_permalink . '" class="post-thumbnail post-thumbnail-' . $position . '">
								<img src="' . esc_url($thumbnail_url) . '" alt="' . $post_title . '" width="' . $thumbnail_width . '" height="' . $thumbnail_height . '" />
							</a>';	
					...
					if ( $this->get_setting('show-titles', true) ) {
	...
	function display_404() {
		...
		$post_id = 'system-404';
		$post_class = 'page system-page system-404';
	...
	function parse_meta($meta) {
		...
		$datetime = ($date_format != 'wordpress-default') ? get_the_time($date_format) : get_the_date();
		...
		$edit_format = $this->get_setting('edit-link-format', ' | %edit_link%');
