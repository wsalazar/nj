...
class HeadwayBlocks {
	...
	public static function register_block_types() {
		...
		foreach ( $headway_unregistered_block_types as $class => $block_type_url ) {
			...
			if ( !class_exists($class) )
				return new WP_Error('block_class_does_not_exist', __('The block class being registered does not exist.', $class));

			$block = new $class();

			if ( $block_type_url )
				$block->block_type_url = $block_type_url;
	...
	public static function load_core_blocks() {
		...
		foreach ( apply_filters('headway_block_types_core', self::$core_blocks['core']) as $block )
			require_once HEADWAY_LIBRARY . '/blocks/core/' . $block . '/' . $block . '.php';
					
		//Load blocks from plugins folder
		foreach ( apply_filters('headway_block_types_plugins', self::$core_blocks['plugins']) as $block )
			require_once HEADWAY_LIBRARY . '/blocks/plugins/' . $block . '/' . $block . '.php';
	...
	public static function output_block_dynamic_js($layout = false) {
		...
		$layout = !$layout ? Headway::get('layout-in-use') : $layout;
	...
	public static function register_block_element_instances() {
		...
		foreach ( $blocks as $block ) {
			...
			$name = Headway::get('alias', $block['settings'], $default_name);			
	...
	public static function display_block($block, $where = null) {
		...
		if ( !$block_type_settings = Headway::get($block['type'], $block_types, array()) ) {
		...
		$custom_css_classes = str_replace('  ', ' ', str_replace(',', ' ', htmlspecialchars(strip_tags(Headway::get('css-classes', $block['settings'], '')))));
		...
		$block_classes[] = ( Headway::get('fixed-height', $block_type_settings, false) !== true ) ? 'block-fluid-height' : 'block-fixed-height';
		
		//Block Styles
		if ( HEADWAY_CHILD_THEME_ACTIVE && $block_style = Headway::get(HeadwayChildThemeAPI::$id . '-block-style', $block['settings']) ) {
						
			$block_style_classes = explode(' ', HeadwayChildThemeAPI::$block_styles[$block_style]['class']);
		...
		if ( HeadwayResponsiveGrid::is_active() ) {
			...
			$responsive_block_hiding = Headway::get('responsive-block-hiding', $block['settings'], array());
		...
		$block_tag = ( $html_tag = Headway::get('html-tag', $block_type_settings) ) ? $html_tag : 'div';
		
		//If we're on the Grid (TRON), then we need to use the original block ID so the grid knows which block it's actually modifying.  
		//Otherwise, just use either the regular or mirrored ID.
		$block_id_for_id_attr = ( $where != 'grid' ) ? $block['id'] : $original_block_id;
		...
		if ( $where != 'grid' ) {
			...
			echo '<' . $block_tag . ' id="block-' . $block_id_for_id_attr . '" class="' . implode(' ', array_filter($block_classes)) . '"' . $block_style_attr . '>';
		...
		//Show the block in the grid
			...
			$show_content_in_grid = Headway::get('show-content-in-grid', $block_type_settings, false);
	...
	public static function block_type_exists($type) {
		...
		if ( !is_array($block_types) )
			return new WP_Error('blocks_array_does_not_exist', __('The Headway blocks array does not exist.', $this));
