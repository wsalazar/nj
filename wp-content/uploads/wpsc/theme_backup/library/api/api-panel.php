...
function headway_register_visual_editor_panel_callback($class) {
	...
	if ( !class_exists($class) )
		return new WP_Error('panel_class_does_not_exist', __('The panel class being registered does not exist.', $class));
...
abstract class HeadwayVisualEditorPanelAPI {
	...
	public function register() {
		...
		//Add panel to search
		$GLOBALS['headway_search_panels'][$this->id] = array(
			'name' => $this->name,
			'mode' => $this->mode
		);
		...
		if ( $this->tabs && $this->inputs ) {
			...
			echo '<ul class="sub-tabs">';
				...
				foreach ($this->tabs as $id => $name) {
					...
					//Add tab to search
					$GLOBALS['headway_search_panels'][$this->id]['tabs'][$id] = array(
						'name' => $name
					);
	...
	public function create_inputs($tab) {
		...
		if ( isset($inputs) && is_array($inputs) ) {
			...
			if ( !isset($inputs[$tab]) || !is_array($inputs[$tab]) )
				return new WP_Error('panel_no_inputs', __('There are no inputs registered for this tab.', $tab));
			...
			foreach($inputs[$tab] as $name => $input) {
				...
				if ( isset($input['label']) ) {
					...
					$input['tooltip'] = (isset($input['tooltip'])) ? $input['tooltip'] : '';
					
					$GLOBALS['headway_search_panels'][$this->id]['tabs'][$tab]['inputs'][$input['name']] = array(
						'label' => $input['label'],
				...
				if ( method_exists(&$this, 'input_' . str_replace('-', '_', $input['type'])) ) {
					...
					echo '<div class="input input-' . $input['type'] . '" id="input-' . $input['name'] . '">';
						...
						if ( $this->is_block && isset($this->block) )
						...
						//Set up the callback
						$block_js_var = ($this->is_block && $this->block_id) ? 'var block = $i(\'#block-' . $this->block_id . '\');' : 'var block = null;';
	...
	public function input_checkbox($input) {
		...
		echo '
			...
			<div class="input-right">
				...
				<input ' . $input['attributes'] . ' type="hidden" />
			</div>
		';
