...
class HeadwayElementAPI {
	...
	public static function get_inherit_location($element, $special_element_type = false, $special_element_meta = false) {
		...
		if ( Headway::get('inherit-location', $element_query, false) ) {
	...
	public static function register_element($args) {
		...
		if ( !is_array($args) )
			return new WP_Error('hw_elements_register_element_args_not_array', __('Arguments must be an array for this element.'), $args);
		...
		if ( $item['id'] === null )
			return new WP_Error('hw_elements_register_element_no_id', __('An ID is required for this element.'), $item);
			
		if ( $item['name'] === null )
			return new WP_Error('hw_elements_register_element_no_name', __('A name is required for this element.'), $item);	
			
		if ( $item['group'] === null && $item['default-element'] === false )
			return new WP_Error('hw_elements_register_element_no_group', __('A group is required for this element.'), $item);	
			
		if ( $item['selector'] === null  && $item['default-element'] === false )
			return new WP_Error('hw_elements_register_element_no_selector', __('A CSS selector is required for this element.'), $item);
			
		if ( $item['properties'] === array() )
			return new WP_Error('hw_elements_register_element_no_properties', __('Properties are required for this element.'), $item);	
	...
	public static function register_element_instance($args) {
		...
		if ( !is_array($args) )
			return new WP_Error('hw_elements_register_element_instance_args_not_array', __('Arguments must be an array for this element instance.'), $args);
		...
		//If requirements are not met, throw errors
		if ( $item['id'] === null )
			return new WP_Error('hw_elements_register_element_instance_no_id', __('An ID is required for this element instance.'), $item);
			
		if ( $item['name'] === null )
			return new WP_Error('hw_elements_register_element_instance_no_name', __('A name is required for this element instance.'), $item);	
			
		if ( $item['group'] === null )
			return new WP_Error('hw_elements_register_element_instance_no_group', __('A group is required for this element instance.'), $item);	
		
		if ( $item['element'] === null )
			return new WP_Error('hw_elements_register_element_instance_no_parent', __('A parent element is required for this element instance.'), $item);
			
		if ( $item['selector'] === null )
			return new WP_Error('hw_elements_register_element_instance_no_selector', __('A CSS selector is required for this element instance.'), $item);
		...
		if ( !isset($destination['supports_instances']) || $destination['supports_instances'] === false || !is_array($destination['instances']) )
			return new WP_Error('hw_elements_register_element_instance_not_support', __('The element specified does not support instances.'), $item);
	...
	public static function register_group($id, $name) {
		...
		if ( isset(self::$groups[$id]) && isset(self::$elements[$id]) )
			return new WP_Error('hw_elements_register_group_already_exists', __('The group being registered already exists.'), $id);
	...
	public static function deregister_element($args) {
		...
		if ( !is_array($args) )
			return new WP_Error('hw_elements_deregister_element_args_not_array', __('Arguments must be an array for deregistering this element.'), $args);
		...
		//If requirements are not met, throw errors
		if ( $item['group'] === null )
			return new WP_Error('hw_elements_deregister_element_no_group', __('A group is required for deregistering this element.'), $item);
			
		if ( $item['id'] === null )
			return new WP_Error('hw_elements_deregister_element_no_id', __('An ID is required for deregistering this element.'), $item);
	...
	public static function deregister_element_instance($args) {
		...
		if ( !is_array($args) )
			return new WP_Error('hw_elements_deregister_element_instance_args_not_array', __('Arguments must be an array for deregistering this element instance.'), $args);
		...
		//If requirements are not met, throw errors
		if ( $item['group'] === null )
			return new WP_Error('hw_elements_deregister_element_instance_no_group', __('A group is required for deregistering this element instance.'), $item);
			
		if ( $item['id'] === null )
			return new WP_Error('hw_elements_deregister_element_instance_no_id', __('An instance ID is required for deregistering this element instance.'), $item);
			
		if ( $item['element'] === null )
			return new WP_Error('hw_elements_deregister_element_instance_no_element', __('An element is required for deregistering this element instance.'), $item);
	...
	public static function deregister_group($id) {
		...
		if ( !isset(self::$elements[$id]) && !isset(self::$groups[$id]) )
			return new WP_Error('hw_elements_deregister_group_does_not_exist', __('The group being deregistered does not exist.'), $id);
