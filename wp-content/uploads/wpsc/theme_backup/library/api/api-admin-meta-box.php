...
function headway_register_admin_meta_box_callback($class) {
	...
	if ( !class_exists($class) )
		return new WP_Error('meta_box_class_does_not_exist', __('The meta box class being registered does not exist.', $class));
...
abstract class HeadwayAdminMetaBoxAPI {
	...
	public function register() {
		...
		//Add the prefix to the and name
		$this->id = 'headway-admin-meta-box-' . $this->id;
		$this->name = 'Headway: ' . $this->name;
	...
	public function save($post_ID, $post) {
		...
		//Ruh-roh, bad nonce
		if ( !wp_verify_nonce(Headway::post($this->id . '_nonce'), md5($this->id)) )
		  	return false;			
